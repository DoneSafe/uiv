import { openBlock as a, createElementBlock as h, normalizeClass as m, renderSlot as p, computed as w, createBlock as S, resolveDynamicComponent as D, withCtx as f, createElementVNode as s, withModifiers as y, resolveComponent as H, createVNode as g, createCommentVNode as b, withDirectives as C, normalizeStyle as B, withKeys as k, vModelText as z, createTextVNode as T, toDisplayString as I } from "vue";
function N(e) {
  return typeof e < "u" && e !== null;
}
const P = {
  uiv: {
    datePicker: {
      clear: "Clear",
      today: "Today",
      month: "Month",
      month1: "January",
      month2: "February",
      month3: "March",
      month4: "April",
      month5: "May",
      month6: "June",
      month7: "July",
      month8: "August",
      month9: "September",
      month10: "October",
      month11: "November",
      month12: "December",
      year: "Year",
      week1: "Mon",
      week2: "Tue",
      week3: "Wed",
      week4: "Thu",
      week5: "Fri",
      week6: "Sat",
      week7: "Sun"
    },
    timePicker: {
      am: "AM",
      pm: "PM"
    },
    modal: {
      cancel: "Cancel",
      ok: "OK"
    },
    multiSelect: {
      placeholder: "Select...",
      filterPlaceholder: "Search..."
    }
  }
};
let W = P, A = function() {
  return "$t" in this ? this.$t.apply(this, arguments) : null;
};
const U = function(e, t) {
  t = t || {};
  let n;
  try {
    if (n = A.apply(this, arguments), N(n) && !t.$$locale)
      return n;
  } catch {
  }
  const o = e.split(".");
  let u = t.$$locale || W;
  for (let l = 0, d = o.length; l < d; l++) {
    const i = o[l];
    if (n = u[i], l === d - 1)
      return n;
    if (!n)
      return "";
    u = n;
  }
  return "";
}, j = {
  // <a> props
  href: { type: String, default: void 0 },
  target: { type: String, default: void 0 },
  // <router-link> props
  to: { type: null, default: void 0 },
  replace: { type: Boolean, default: !1 },
  append: { type: Boolean, default: !1 },
  exact: { type: Boolean, default: !1 }
}, O = {
  __name: "BtnGroup",
  props: {
    size: { type: String, default: void 0 },
    vertical: { type: Boolean, default: !1 },
    justified: { type: Boolean, default: !1 }
  },
  setup(e) {
    return (t, n) => (a(), h("div", {
      class: m({
        "btn-group": !e.vertical,
        "btn-group-vertical": e.vertical,
        "btn-group-justified": e.justified,
        [`btn-group-${e.size}`]: e.size
      }),
      role: "group",
      "data-toggle": "buttons"
    }, [
      p(t.$slots, "default")
    ], 2));
  }
}, E = ["href", "target"], K = ["type", "checked", "disabled"], J = ["type", "disabled"], F = ["type", "disabled"], L = {
  __name: "Btn",
  props: {
    ...j,
    justified: { type: Boolean, default: !1 },
    type: { type: String, default: "default" },
    nativeType: { type: String, default: "button" },
    size: { type: String, default: void 0 },
    block: { type: Boolean, default: !1 },
    active: { type: Boolean, default: !1 },
    disabled: { type: Boolean, default: !1 },
    // <input> props
    modelValue: { type: null, default: null },
    inputValue: { type: null, default: null },
    inputType: {
      type: String,
      validator(e) {
        return e === "checkbox" || e === "radio";
      },
      default: void 0
    }
  },
  emits: ["update:modelValue", "click"],
  setup(e, { emit: t }) {
    const n = e, o = w(
      () => n.inputType === "checkbox" ? n.modelValue.indexOf(n.inputValue) >= 0 : n.modelValue === n.inputValue
    ), u = w(() => ({
      btn: !0,
      active: n.inputType ? o.value : n.active,
      disabled: n.disabled,
      "btn-block": n.block,
      [`btn-${n.type}`]: !!n.type,
      [`btn-${n.size}`]: !!n.size
    }));
    function l(i) {
      n.disabled && i instanceof Event ? (i.preventDefault(), i.stopPropagation()) : t("click", i);
    }
    function d() {
      if (n.inputType === "checkbox") {
        const i = n.modelValue.slice();
        o.value ? i.splice(i.indexOf(n.inputValue), 1) : i.push(n.inputValue), t("update:modelValue", i);
      } else
        t("update:modelValue", n.inputValue);
    }
    return (i, V) => i.href ? (a(), h("a", {
      key: 0,
      href: i.href,
      target: i.target,
      role: "button",
      class: m(u.value),
      onClick: l
    }, [
      p(i.$slots, "default")
    ], 10, E)) : i.to ? (a(), S(D("RouterLink"), {
      key: 1,
      to: i.to,
      class: m(u.value),
      event: e.disabled ? "" : "click",
      replace: i.replace,
      append: i.append,
      exact: i.exact,
      role: "button",
      onClick: l
    }, {
      default: f(() => [
        p(i.$slots, "default")
      ]),
      _: 3
    }, 8, ["to", "class", "event", "replace", "append", "exact"])) : e.inputType ? (a(), h("label", {
      key: 2,
      class: m(u.value),
      onClick: l
    }, [
      s("input", {
        autocomplete: "off",
        type: e.inputType,
        checked: o.value,
        disabled: e.disabled,
        onInput: V[0] || (V[0] = y(() => {
        }, ["stop"])),
        onChange: d
      }, null, 40, K),
      p(i.$slots, "default")
    ], 2)) : e.justified ? (a(), S(O, { key: 3 }, {
      default: f(() => [
        s("button", {
          class: m(u.value),
          type: e.nativeType,
          disabled: e.disabled,
          onClick: l
        }, [
          p(i.$slots, "default")
        ], 10, J)
      ]),
      _: 3
    })) : (a(), h("button", {
      key: 4,
      class: m(u.value),
      type: e.nativeType,
      disabled: e.disabled,
      onClick: l
    }, [
      p(i.$slots, "default")
    ], 10, F));
  }
};
function v(e, t) {
  let n = e.toString();
  for (let o = t - n.length; o > 0; o--)
    n = "0" + n;
  return n;
}
const R = (e, t) => {
  const n = e.__vccOpts || e;
  for (const [o, u] of t)
    n[o] = u;
  return n;
}, x = 23, c = 0, M = 59, r = 12, Y = {
  components: { Btn: L },
  props: {
    modelValue: { type: Date, required: !0 },
    showMeridian: { type: Boolean, default: !0 },
    min: { type: null, default: void 0 },
    max: { type: null, default: void 0 },
    hourStep: { type: Number, default: 1 },
    minStep: { type: Number, default: 1 },
    readonly: { type: Boolean, default: !1 },
    controls: { type: Boolean, default: !0 },
    iconControlUp: { type: String, default: "glyphicon glyphicon-chevron-up" },
    iconControlDown: {
      type: String,
      default: "glyphicon glyphicon-chevron-down"
    },
    inputWidth: { type: Number, default: 50 }
  },
  emits: ["update:modelValue"],
  data() {
    return {
      hours: 0,
      minutes: 0,
      meridian: !0,
      hoursText: "",
      minutesText: ""
    };
  },
  computed: {
    inputStyles() {
      return {
        width: `${this.inputWidth}px`
      };
    }
  },
  watch: {
    modelValue(e) {
      this.updateByValue(e);
    },
    showMeridian(e) {
      this.setTime();
    },
    hoursText(e) {
      if (this.hours === 0 && e === "")
        return;
      const t = parseInt(e);
      this.showMeridian ? t >= 1 && t <= r && (this.meridian ? this.hours = t === r ? 0 : t : this.hours = t === r ? r : t + r) : t >= c && t <= x && (this.hours = t), this.setTime();
    },
    minutesText(e) {
      if (this.minutes === 0 && e === "")
        return;
      const t = parseInt(e);
      t >= c && t <= M && (this.minutes = t), this.setTime();
    }
  },
  mounted() {
    this.updateByValue(this.modelValue);
  },
  methods: {
    t: U,
    updateByValue(e) {
      if (isNaN(e.getTime())) {
        this.hours = 0, this.minutes = 0, this.hoursText = "", this.minutesText = "", this.meridian = !0;
        return;
      }
      this.hours = e.getHours(), this.minutes = e.getMinutes(), this.showMeridian ? this.hours >= r ? (this.hours === r ? this.hoursText = this.hours + "" : this.hoursText = v(this.hours - r, 2), this.meridian = !1) : (this.hours === c ? this.hoursText = r.toString() : this.hoursText = v(this.hours, 2), this.meridian = !0) : this.hoursText = v(this.hours, 2), this.minutesText = v(this.minutes, 2), this.$refs.hoursInput.value = this.hoursText, this.$refs.minutesInput.value = this.minutesText;
    },
    addHour(e) {
      e = e || this.hourStep, this.hours = this.hours >= x ? c : this.hours + e;
    },
    reduceHour(e) {
      e = e || this.hourStep, this.hours = this.hours <= c ? x : this.hours - e;
    },
    addMinute() {
      this.minutes >= M ? (this.minutes = c, this.addHour(1)) : this.minutes += this.minStep;
    },
    reduceMinute() {
      this.minutes <= c ? (this.minutes = M + 1 - this.minStep, this.reduceHour(1)) : this.minutes -= this.minStep;
    },
    changeTime(e, t) {
      this.readonly || (e && t ? this.addHour() : e && !t ? this.reduceHour() : !e && t ? this.addMinute() : this.reduceMinute(), this.setTime());
    },
    toggleMeridian() {
      this.meridian = !this.meridian, this.meridian ? this.hours -= r : this.hours += r, this.setTime();
    },
    onWheel(e, t) {
      this.readonly || (e.preventDefault(), this.changeTime(t, e.deltaY < 0));
    },
    setTime() {
      let e = this.modelValue;
      if (isNaN(e.getTime()) && (e = /* @__PURE__ */ new Date(), e.setHours(0), e.setMinutes(0)), e.setHours(this.hours), e.setMinutes(this.minutes), this.max instanceof Date) {
        const t = new Date(e);
        t.setHours(this.max.getHours()), t.setMinutes(this.max.getMinutes()), e = e > t ? t : e;
      }
      if (this.min instanceof Date) {
        const t = new Date(e);
        t.setHours(this.min.getHours()), t.setMinutes(this.min.getMinutes()), e = e < t ? t : e;
      }
      this.$emit("update:modelValue", new Date(e));
    },
    selectInputValue(e) {
      e.target.setSelectionRange(0, 2);
    }
  }
}, q = {
  key: 0,
  class: "text-center"
}, G = /* @__PURE__ */ s("td", null, " ", -1), Q = { key: 0 }, X = { class: "form-group" }, Z = ["readonly"], $ = /* @__PURE__ */ s("td", null, [
  /* @__PURE__ */ T(" "),
  /* @__PURE__ */ s("b", null, ":"),
  /* @__PURE__ */ T(" ")
], -1), _ = { class: "form-group" }, ee = ["readonly"], te = { key: 0 }, ne = {
  key: 1,
  class: "text-center"
}, ie = /* @__PURE__ */ s("td", null, " ", -1), le = { key: 0 };
function se(e, t, n, o, u, l) {
  const d = H("btn");
  return a(), h("section", {
    onClick: t[14] || (t[14] = y(() => {
    }, ["stop"]))
  }, [
    s("table", null, [
      s("tbody", null, [
        n.controls ? (a(), h("tr", q, [
          s("td", null, [
            g(d, {
              type: "link",
              size: "sm",
              disabled: n.readonly,
              onClick: t[0] || (t[0] = (i) => l.changeTime(1, 1))
            }, {
              default: f(() => [
                s("i", {
                  class: m(n.iconControlUp)
                }, null, 2)
              ]),
              _: 1
            }, 8, ["disabled"])
          ]),
          G,
          s("td", null, [
            g(d, {
              type: "link",
              size: "sm",
              disabled: n.readonly,
              onClick: t[1] || (t[1] = (i) => l.changeTime(0, 1))
            }, {
              default: f(() => [
                s("i", {
                  class: m(n.iconControlUp)
                }, null, 2)
              ]),
              _: 1
            }, 8, ["disabled"])
          ]),
          n.showMeridian ? (a(), h("td", Q)) : b("", !0)
        ])) : b("", !0),
        s("tr", null, [
          s("td", X, [
            C(s("input", {
              ref: "hoursInput",
              "onUpdate:modelValue": t[2] || (t[2] = (i) => u.hoursText = i),
              type: "tel",
              pattern: "\\d*",
              class: "form-control text-center",
              style: B(l.inputStyles),
              placeholder: "HH",
              readonly: n.readonly,
              maxlength: "2",
              size: "2",
              onMouseup: t[3] || (t[3] = (...i) => l.selectInputValue && l.selectInputValue(...i)),
              onKeydown: [
                t[4] || (t[4] = k(y((i) => l.changeTime(1, 1), ["prevent"]), ["up"])),
                t[5] || (t[5] = k(y((i) => l.changeTime(1, 0), ["prevent"]), ["down"]))
              ],
              onWheel: t[6] || (t[6] = (i) => l.onWheel(i, !0))
            }, null, 44, Z), [
              [
                z,
                u.hoursText,
                void 0,
                { lazy: !0 }
              ]
            ])
          ]),
          $,
          s("td", _, [
            C(s("input", {
              ref: "minutesInput",
              "onUpdate:modelValue": t[7] || (t[7] = (i) => u.minutesText = i),
              type: "tel",
              pattern: "\\d*",
              class: "form-control text-center",
              style: B(l.inputStyles),
              placeholder: "MM",
              readonly: n.readonly,
              maxlength: "2",
              size: "2",
              onMouseup: t[8] || (t[8] = (...i) => l.selectInputValue && l.selectInputValue(...i)),
              onKeydown: [
                t[9] || (t[9] = k(y((i) => l.changeTime(0, 1), ["prevent"]), ["up"])),
                t[10] || (t[10] = k(y((i) => l.changeTime(0, 0), ["prevent"]), ["down"]))
              ],
              onWheel: t[11] || (t[11] = (i) => l.onWheel(i, !1))
            }, null, 44, ee), [
              [
                z,
                u.minutesText,
                void 0,
                { lazy: !0 }
              ]
            ])
          ]),
          n.showMeridian ? (a(), h("td", te, [
            T("   "),
            g(d, {
              "data-action": "toggleMeridian",
              disabled: n.readonly,
              onClick: l.toggleMeridian
            }, {
              default: f(() => [
                T(I(u.meridian ? l.t("uiv.timePicker.am") : l.t("uiv.timePicker.pm")), 1)
              ]),
              _: 1
            }, 8, ["disabled", "onClick"])
          ])) : b("", !0)
        ]),
        n.controls ? (a(), h("tr", ne, [
          s("td", null, [
            g(d, {
              type: "link",
              size: "sm",
              disabled: n.readonly,
              onClick: t[12] || (t[12] = (i) => l.changeTime(1, 0))
            }, {
              default: f(() => [
                s("i", {
                  class: m(n.iconControlDown)
                }, null, 2)
              ]),
              _: 1
            }, 8, ["disabled"])
          ]),
          ie,
          s("td", null, [
            g(d, {
              type: "link",
              size: "sm",
              disabled: n.readonly,
              onClick: t[13] || (t[13] = (i) => l.changeTime(0, 0))
            }, {
              default: f(() => [
                s("i", {
                  class: m(n.iconControlDown)
                }, null, 2)
              ]),
              _: 1
            }, 8, ["disabled"])
          ]),
          n.showMeridian ? (a(), h("td", le)) : b("", !0)
        ])) : b("", !0)
      ])
    ])
  ]);
}
const oe = /* @__PURE__ */ R(Y, [["render", se]]);
export {
  oe as default
};

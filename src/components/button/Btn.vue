<template>
  <a v-if="href" :href="href" :target="target" role="button" :class="classes" @click="onClick">
    <slot />
  </a>
  <component
    :is="'RouterLink'"
  ...
  <label v-else-if="inputType" :class="classes" @click="onClick">
    <input autocomplete="off" :type="inputType" :checked="isInputActive" :disabled="disabled" @input.stop @change="onInputChange" />
    <slot />
  </label>
  <btn-group v-else-if="justified">
    <button :class="classes" :type="nativeType" :disabled="disabled" @click="onClick">
      <slot />
    </button>
  </btn-group>
  <button v-else :class="classes" :type="nativeType" :disabled="disabled" @click="onClick">
    <slot />
  </button>
</template>
...
<script setup>
  ...
  const isInputActive = computed(() =>
    props.inputType === 'checkbox' ? props.modelValue.indexOf(props.inputValue) >= 0 : props.modelValue === props.inputValue
  );
